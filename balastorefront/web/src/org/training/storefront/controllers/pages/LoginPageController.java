...
import org.training.facades.facade.DefaultCustomCustomerFacade;
...
import org.training.storefront.controllers.command.validation.CustomRegistrationFormValidator;
...
public class LoginPageController extends AbstractLoginPageController
{
	...
	@Resource(name = "customRegistrationFormValidator")
	private CustomRegistrationFormValidator customRegistrationFormValidator;

	@Resource(name = "defaultCustomCustomerFacade")
	private DefaultCustomCustomerFacade defaultCustomCustomerFacade;
	...
	public String doRegister(@RequestHeader(value = "referer", required = false) final String referer, final RegisterForm form,
			final CustomRegisterForm customRegisterForm,final BindingResult bindingResult, final Model model, final HttpServletRequest request,
			final HttpServletResponse response, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		customRegistrationFormValidator.validate(customRegisterForm, bindingResult);
		return processRegisterUserRequest(referer, customRegisterForm, bindingResult, model, request, response, redirectModel);
	}
	...
	{
		CustomRegisterForm customRegisterForm= (CustomRegisterForm) form;
		if (bindingResult.hasErrors())
		{
			form.setTermsCheck(false);
			customRegisterForm.setNewBusinessUser(false);
		...
		data.setPlace(customRegisterForm.getPlace());
		data.setNewBusinessUser(customRegisterForm.isNewBusinessUser());
		try
		{
